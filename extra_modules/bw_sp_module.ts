...
export class SilentPayment {
  ...
  createTransaction(utxos: UTXO[], targets: Target[], network: bitcoin.networks.Network = bitcoin.networks.bitcoin): Target[] {
    ...
    for (const group of silentPaymentGroups) {
      ...
      for (const [Bm, amount, i] of group.BmValues) {
        ...
        const address = SilentPayment.pubkeyToAddress(Pmk.slice(1).toString("hex"), network);
  ...
  static pubkeyToAddress(hex: string, network: bitcoin.networks.Network = bitcoin.networks.bitcoin): string {
